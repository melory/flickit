/**
  *
  * BaseLine Language Model for visual words only with the Large grain size case with P(L)'s influence.
  *
  * here we made some improvements on the basis of 'BLLM_vword_only_L.c' and
  * it may has a smaller time complexity
  *
  * 程序会输出testing的2K张照片和tunning的2K张照片在每个cell里面的p(T|L)值，考虑P(L)d的影响
...
main()
{
	int i, j, k, cellid, cellidActual, centerid, vwordNum, **freq, vword_cell[CELLNUM], vword_globe, a_vword_total[CENTERNUM], vword_seq[100000], PL[CELLNUM];
	char id[11], name[40];
	double lat, lon, PTL;
	FILE *fp_modeling, *fp_tunning, *fp_testing, *fp_PTL, *fp_PL;
	...
	if((fp_PL = fopen("key_each_cell.txt", "r")) == NULL)
	{
		printf("openning file 'key_each_cell.txt' failed.\n");
		return -1;
	}

	while(fscanf(fp_PL, "%d%d", &cellid, &i) != EOF)
		PL[cellid] = i + 1;
	...
	while(fscanf(fp_tunning, "%s%lf%lf", id, &lat, &lon) != EOF)
	{
		...
		for(i = 0; i < CELLNUM; i++)
		{
			...
			PTL = (PTL + log((double)(PL[i]))) / vwordNum;
			fprintf(fp_PTL, "%d %lf\n", i, PTL);
		}
		fclose(fp_PTL);
	}
